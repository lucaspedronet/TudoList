...
export default function App() {
  ...
  //se usuario tentar adicionar tarefa sem preencher a mesma, app dispara alerta.
  function handleSubmit() { //entrada de dados 
    if (!tarefa) {
      return Alert.alert('Por favor digite uma tarefa!');
    }

    //se usuario tentar inserir a mesma tarefa 2 vezes, app dispara alerta
    ...
    //formato de dados recebidos do usuário
    const novaTarefa: TarefaType = {
      feito: false, //tarefa não concluida
  ...
  function handleDelete(id: string) { //exclusão de dados, caso toque icone da lixeira
    Alert.alert(
      'REMOVER',
      ...
      <View style={styled.content}>
        ...
        <View style={{ flexDirection: 'row', height: 71 }}>
          <View style={{ flex: 1, flexDirection: 'row', top: 32, height: 19 }}>
            <Text style={styled.criadas}>Criadas/Pendentes</Text>
            <View
              style={{
                height: 22,
                width: 22,
        ...
        <ScrollView style={styled.contentlist}>
          {listaDeTarefas.length === 0 ? (
            <View style={{ flex: 1, alignItems: 'center', gap: 75}}>
...
const styled = StyleSheet.create({
  ...
  title3: {
    ...
    textAlign: 'center',
    fontWeight: '400',
    top: 4,//fica fora da tela com 64
  },
  subtitle: {
    color: '#6B6B6B',
    fontSize: 16,
